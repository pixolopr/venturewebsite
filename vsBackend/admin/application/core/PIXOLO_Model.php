...
class PIXOLO_Model extends CI_Model
{
   ...
   public function record_count($tablename) {
    ...
    public function getall($limit, $start, $order) {
    ...
    {
        $db=$this->db->database;
        $query = $this->db->query("SELECT `TABLE_NAME` FROM INFORMATION_SCHEMA.COLUMNS WHERE `TABLE_SCHEMA`='$db' AND `COLUMN_COMMENT` != '' GROUP BY `TABLE_NAME`")->result();
        return $query;
    }
